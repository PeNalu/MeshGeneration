...
{
    public static List<List<Vector2>> ClipPolygons(List<Vector2> polyVector, List<Vector2> clipPolyVector, Geometry.BooleanOperation booleanOperation)
    {
        ...
        for (int i = 0; i < poly.Count; i++)
        {
            ...
            int iPlusOne = Geometry.ClampListIndex(i + 1, poly.Count);
            ...
            {
                int jPlusOne = Geometry.ClampListIndex(j + 1, clipPoly.Count);
                ...
                if (LineLineIntersection.AreLinesIntersecting(a, b, c, d, true))
                {
                    hasFoundIntersection = true;

                    Vector2 intersectionPoint2D = LineLineIntersection.GetLineLineIntersectionPoint(a, b, c, d);
        ...
        if (hasFoundIntersection)
        {
            ...
            if (booleanOperation == Geometry.BooleanOperation.Intersection)
            {
            ...
            else if (booleanOperation == Geometry.BooleanOperation.Difference)
            {
            ...
            else if (booleanOperation == Geometry.BooleanOperation.ExclusiveOr)
            {
                //Whats outside of the polygon that doesnt intersect
                List<ClipVertex> outsidePolyVertices = GetClippedPolygon(poly, false);
            ...
            else if (booleanOperation == Geometry.BooleanOperation.Union)
            {
                //Where the two polygons intersect
                List<ClipVertex> intersectionVertices = GetClippedPolygon(poly, true);
    ...
    private static bool IsPolygonInsidePolygon(List<Vector2> polyOne, List<Vector2> polyTwo)
    {
        ...
        {
            if (Geometry.IsPointInPolygon(polyTwo, polyOne[i]))
            {
    ...
    private static List<ClipVertex> InitDataStructure(List<Vector2> polyVector)
    {
        ...
        {
            int iPlusOne = Geometry.ClampListIndex(i + 1, poly.Count);
            int iMinusOne = Geometry.ClampListIndex(i - 1, poly.Count);
    ...
    private static void MarkEntryExit(List<ClipVertex> poly, List<Vector2> clipPolyVector)
    {
        ...
        bool isInside = Geometry.IsPointInPolygon(clipPolyVector, poly[0].coordinate);
